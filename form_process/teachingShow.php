...
<?php
	...
	if($name == 'lect'){
		$sql="SELECT * FROM teach_lstp WHERE Teach_LSTP_Course = '".$val."'";
	...
	?>
         ...
         <br><label>Consulted</label>
		  ...
		  <br><label>Additional Resources Provided : </label>
				 ...
				 <input type="text" class="form-control required" name="resrc" title="Please Fill this information" value="<?php echo $resources; ?>"/>	
	 ...
	 <?php
		...
		if($name == 'dut'){
			$sql="SELECT * FROM teach_edap WHERE User_Id = '$userId' and Teach_EDAP_TED = '$val'";
			$result = mysqli_query($con,$sql) or die('Error'.mysqli_error($con));
			$row = mysqli_fetch_array($result);
