...
<?php
	if(isset($_POST['lectSave'])){
		...
		$sql1 = "SELECT * FROM teach_lstp where User_Id='$user_id' and Teach_LSTP_Course='$courseName'";
		...
		if(!empty($row['User_Id']) and !empty($row['Teach_LSTP_Course'])){
			...
			if($result1){
				header('location:teachingLearning.php');
			}
			else{
			...
			$insertQuery = "Insert Into teach_lstp values('$user_id','','$courseName','$level','$modes','$clsAllocated','$clsConducted','$practicals','$clsPercent','$clsTaken','$teachingLoad')";
	...
	if(isset($_POST['lectDelete'])){
		...
		$deleteQuery = "Delete From teach_lstp where User_Id='$user_id' and Teach_LSTP_Course='$courseName'";
...
// Php Script For Updating information for Reading/Instructional Material
	if(isset($_POST['resourceSave'])){
		...
		$sql1 = "SELECT * FROM teach_rimc where User_Id='$user_id' and Teach_RIMC_Course='$readingCourse'";
		$result = mysqli_query($con, $sql1);
		$row = mysqli_fetch_array($result);
		if(!empty($row['User_Id']) and !empty($row['Teach_RIMC_Course'])){
			...
			$insertQuery = "Insert Into teach_rimc values('$user_id','','$readingCourse','$consulted','$prescribed','$resources')";
			$result2 = mysqli_query($con,$insertQuery);
			if($result2){
				header('location:teachingLearning.php');
			}
	...
	if(isset($_POST['resourceDelete'])){
		...
		$deleteQuery = "Delete From teach_rimc where User_Id='$user_id' and Teach_RIMC_Course='$readingCourse'";
		$result = mysqli_query($con,$deleteQuery);
		if($result){
			header('location:teachingLearning.php');
		}
...
// Php Script For Updating information for Teaching-Learning Methodologies Tab
	if(isset($_POST['innovationSave'])){
		...
		if(!empty($row['User_Id']) and !empty($row['Teach_TLM_SD'])){
			$updateQuery = "UPDATE teach_tlm SET Teach_TLM_API='$apiScore' where User_Id='$user_id' and Teach_TLM_SD='$description'" ;
			$result1 = mysqli_query($con,$updateQuery);
			if($result1){
				header('location:teachingLearning.php');
			}
			...
			$insertQuery = "Insert Into teach_tlm values('$user_id','','$description','$apiScore')";
			$result2 = mysqli_query($con,$insertQuery);
			if($result2){
				header('location:teachingLearning.php');
			}
...
// Php Script - Updating information for Examination Duties Assigned and Performed
	if(isset($_POST['dutiesSave'])){
		...
		$sql1 = "SELECT * FROM teach_edap where User_Id='$user_id' and Teach_EDAP_TED='$type'";
		$result = mysqli_query($con, $sql1);
		$row = mysqli_fetch_array($result);
		if(!empty($row['User_Id']) and !empty($row['Teach_EDAP_TED'])){
			$updateQuery = "UPDATE teach_edap SET Teach_EDAP_DA='$assigned', Teach_EDAP_ECO='$extent', Teach_EDAP_API='$apiScore' where User_Id='$user_id' and Teach_EDAP_TED='$type'" ;
			$result1 = mysqli_query($con,$updateQuery);
			if($result1){
				header('location:teachingLearning.php');
			}
			...
			$insertQuery = "Insert Into teach_edap values('$user_id','','$type','$assigned','$extent','$apiScore')";
			$result2 = mysqli_query($con,$insertQuery);
			if($result2){
				header('location:teachingLearning.php');
			}
	...
	if(isset($_POST['dutiesDelete'])){
		...
		$deleteQuery = "Delete From teach_edap where User_Id='$user_id' and Teach_EDAP_TED='$type'";
		$result = mysqli_query($con,$deleteQuery);
		if($result){
			header('location:teachingLearning.php');
		}
